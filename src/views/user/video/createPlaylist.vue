      ...
      >
        <mo-dashboard-body :loading="isLoading">
          ...
          <template #footer-left>
            <b-form-select  v-if="isNew" v-model="compression" :options="options"></b-form-select>
          </template>
          <template v-if="filecount>5" #footer-right>
            <mo-button
              v-if="isNew"
              type="submit"
              title="Create"
...
import MoVideoPlayer from "@/components/pure/MoVideoPlayer2";
  ...
  methods: {
    ...
    onCreate() {
     ...
     // for (var i = 0; i < this.videoFiles.length; i++) {
      ...
      formData.append("video1", this.videoFiles[0].video);
      formData.append("video2", this.videoFiles[1].video);
      formData.append("video3", this.videoFiles[2].video);
      formData.append("video4", this.videoFiles[3].video);
      formData.append("video5", this.videoFiles[4].video);
      //console.log(this.videoFiles[i].video)
      //}
       /*formData.append("orientation1", this.videoFiles[0].rotation);
      formData.append("orientation2", this.videoFiles[1].rotation);
      formData.append("orientation3", this.videoFiles[2].rotation);
      formData.append("orientation4", this.videoFiles[3].rotation);
      formData.append("orientation5", this.videoFiles[4].rotation);*/
      formData.append("orientation", this.orientation);
     
      const { response, cancel } = createPlaylistVideo(formData, progress => {
