<template>
  <validation-observer class="h-100 flex-grow-1" v-slot="{ handleSubmit }">
    <mo-prompt-modal
      modal-id="delete-decorating-modal"
      message="Are you sure you want to delete this Decorating house ?"
      @right="onDelete"
    />
    <form
      ...
      <mo-dashboard-body :loading="isLoading">
        ...
        <template #footer-left>
          <mo-button
            ...
            @click="$bvModal.show('delete-decorating-modal')"
            title="Delete"
            variant="danger"
          />
        </template>
...
export default {
  ...
  computed: {
    isNew() {
      return this.decoratingHouseId === "new";
    },
    headerTitle() {
      return this.isNew ? `Add decorating house` : `Edit decorating house`;
    },
    decoratingHouseData() {
      return {
  ...
  methods: {
    ...
    onCreate() {
      this.handleCreate(
        createDecoratingHouse(this.decoratingHouseData),
        "Decorating House"
      ).then(() => {
        this.$emit("end");
      });
    ...
    onUpdate() {
      this.handleUpdate(
        updateDecoratingHouse(this.decoratingHouseId, this.decoratingHouseData),
        "Decorating House"
      ).then(() => this.$emit("end", true));
    },
    onDelete() {
      this.handleDelete(
        deleteDecoratingHouse(this.decoratingHouseId),
        "Decorating house"
      ).then(() => {
        this.$emit("end");
      });
    ...
    fetchData() {
      if (!this.isNew) {
        this.handleFetch(getDecoratingHouse(this.decoratingHouseId)).then(
          data => {
